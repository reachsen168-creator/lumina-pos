...
import { ArrowLeft, Plus, Minus, Trash2, Search, History } from "lucide-react";
    ...
    if (customerName.trim()) {
      try {
        ...
        if (lastSale && lastSale.hasHistory) {
          ...
          toast({ title: `Applied last sale price: $${defaultPrice}`, description: `Last bought on ${lastSale.date?.split('T')[0]}` });
        }
      } catch (e) {
        // ignore
      }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-24">
      <div className="flex items-center gap-4 mb-6">
        <Button variant="ghost" size="icon" onClick={() => setLocation("/sales")} className="rounded-xl bg-card border border-border">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6 shadow-sm border-none ring-1 ring-border rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Walk-in Customer" className="h-11 rounded-xl bg-muted/30" />
              </div>
              <div className="space-y-2">
                <Label>Date *</Label>
                <Input type="date" value={date} onChange={e => setDate(e.target.value)} className="h-11 rounded-xl bg-muted/30" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={note} onChange={e => setNote(e.target.value)} placeholder="Optional note" className="h-11 rounded-xl bg-muted/30" />
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6 shadow-sm border-none ring-1 ring-border rounded-2xl overflow-visible">
            ...
            <div className="mt-8 space-y-3">
              ...
              {items.map((item, idx) => (
                <div key={item.tempId} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl border border-border bg-card hover:border-accent/30 transition-colors">
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full">
                      <div className="flex items-center">
                        ...
                        <div className="relative">
                          ...
                          <Input type="number" step="0.01" value={item.price} onChange={e => updateItem(idx, 'price', Number(e.target.value))} className="w-24 h-9 pl-6 rounded-lg" />
                        </div>
                      </div>
                      <div className="flex items-center">
                        ...
                        <div className="flex items-center bg-muted/50 rounded-lg border border-border">
                          <button type="button" onClick={() => updateItem(idx, 'qty', Math.max(1, item.qty - 1))} className="w-9 h-9 flex items-center justify-center hover:bg-background rounded-l-lg"><Minus className="w-4 h-4" /></button>
                          <input type="number" value={item.qty} onChange={e => updateItem(idx, 'qty', Number(e.target.value) || 1)} className="w-12 h-9 text-center bg-transparent border-none focus:ring-0 text-sm font-bold" />
                          <button type="button" onClick={() => updateItem(idx, 'qty', item.qty + 1)} className="w-9 h-9 flex items-center justify-center hover:bg-background rounded-r-lg"><Plus className="w-4 h-4" /></button>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-border sm:border-0">
                    <div className="text-right sm:mr-4">
                      ...
                      <p className="font-display font-bold text-lg text-foreground">${(item.price * item.qty).toFixed(2)}</p>
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => removeItem(idx)} className="text-muted-foreground hover:text-red-500 hover:bg-red-50 h-10 w-10 rounded-xl shrink-0">
        ...
        <div className="lg:col-span-1">
          <Card className="p-6 shadow-xl border-none bg-gradient-to-b from-primary to-primary/95 text-primary-foreground sticky top-24 rounded-2xl">
            <h3 className="font-display font-bold text-xl mb-6 text-primary-foreground/90 border-b border-white/10 pb-4">Summary</h3>
